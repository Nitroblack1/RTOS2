...
pub unsafe extern "C" fn timer() -> ! {
    // Direct RTT log to bypass syscall system - DISABLED for benchmark focus
    ...
    loop {
        unsafe {
            ...
            // Approximate 1 second based on loop iterations (reduced frequency for benchmark)
            if TIMER_TICKS % 20000 == 0 {
